...
Источник хранит состояние наведения через `useState`, предыдущий DOM-узел через `useRef` и создаёт callback-ref, который переносит обработчики между элементами.
